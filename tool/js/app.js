...
function ToggleOldView() { if(isOldView == false) { isOldView = true; } else { isOldView = false;  } LoadFile(currentlySelectedFile); }
...
function LoadFile(fileName) {
  ...
  if(isRaw){ LoadRawFile(fileName); }
  else if(isOldView){ LoadOldConfiguredFile(fileName); }
  else { LoadConfiguredFile(fileName); }
...
function LoadOldConfiguredFile(fileName) {
  if(fileName == 'None'){ $("#browser").html('<pre id="browser-data">Please Select A File First!</pre>'); }
  else {
    readTextFile("../../data/logs/" + fileName, function(text) {
  ...
  if(fileName == 'None'){ $("#browser").html('<pre id="browser-data">Please Select A File First!</pre>'); }
    ...
    readTextFile("../../data/logs/" + fileName, function(text) {
      ...
      for(i in data) {
        ...
        var type = JSON.stringify(data[i].Type).split('"').join('');
        var log = JSON.stringify(data[i].Log).split('"').join('');
        if(type == 'Command') { commandData.push({ dateTime, type, log }); }
        if(type == 'Server') { serverData.push({ dateTime, type, log }); }
        if(type == 'Account') { accountData.push({ dateTime, type, log }); }
        if(type == 'Clans') { clansData.push({ dateTime, type, log }); }
        if(type == 'Warning') { warningData.push({ dateTime, type, log }); }
        if(type == 'Error') { errorData.push({ dateTime, type, log }); }
      }

      $("#browser").html(`
        <div class='command-table'>${ commandData.map(log => (`<div> ${ log.log }</div>`)).join("") }</div>
        <div class='server-table'>${ serverData.map(log => (`<div> ${ log.log } </div>`)).join("") }</div>
        <div class='accounts-table'>${ accountData.map(log => (`<div> ${ log.log } </div>`)).join("") }</div>
        <div class='clans-table'>${ clansData.map(log => (`<div> ${ log.log } </div>`)).join("") }</div>
        <div class='warning-table'>${ warningData.map(log => (`<div> ${ log.log } </div>`)).join("") }</div>
        <div class='error-table'>${ errorData.map(log => (`<div> ${ log.log } </div>`)).join("") }</div>
      `);
    });
    currentlySelectedFile = fileName;
  }
